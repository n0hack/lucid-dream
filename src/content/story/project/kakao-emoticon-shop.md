...
- 홈 배너 이미지에 [Slick](https://kenwheeler.github.io/slick/) 라이브러리 적용
...
무한 스크롤을 구현할 때는 `Intersection Observer` API 이전에 `window` 객체에 스크롤 이벤트를 등록하여 구현하는 방법이 있었습니다. 하지만 Web API 중 하나인 `Intersection Observer`를 이용하면 비동기적으로 작업하기 때문에 성능에 있어 많은 이점을 가져올 수 있습니다. 프로젝트를 진행하면서 두 방법의 차이를 알게 되었고, 이제는 `Intersection Observer` API를 적절하게 사용할 수 있게 되었습니다.

### 라이브러리 도움 없이 드래그 스크롤 직접 구현

드래그 스크롤(Drag Scroll)은 모바일 장치에서 터치를 통해 스크롤하던 것을 데스크톱 환경의 웹에서도 사용 가능하도록 한 기술입니다. 기본적으로 터치를 이용하여 스크롤 하는 느낌이기 때문에 터치 스크롤(Touch Scroll)이라 부르는 것이 맞는 것 같지만, 개인적으로는 드래그 스크롤이 부르기 편해 이대로 진행했습니다. 이 기능은 무조건 직접 구현해 보자라는 생각을 갖고 만들었고, 프로젝트 내에서 재사용이 정말 많이 되기 때문에 완성했을 때의 만족도가 매우 높습니다. 이는 다른 프로젝트에서도 사용할 가능성이 있어, 라이브러리로 만들어 배포해 보면 어떨까 합니다.
...
- [카카오 이모티콘샵 클론 웹사이트](https://n0hack.github.io/kakao-emoticon-shop)
- [GitHub 저장소](https://github.com/n0hack/kakao-emoticon-shop)
- [[스토리] 마우스로 터치 스크롤 구현하기](/story/javascript/post/mouse-touch-scroll)
- [[스토리] Intersection Observer API로 무한 스크롤 구현하기](/story/javascript/post/infinite-scroll-intersection-observer)

> 프로젝트를 진행하며 직접 구현해 본 기술들을 각각 하나의 스토리 안에 녹였습니다 😊
